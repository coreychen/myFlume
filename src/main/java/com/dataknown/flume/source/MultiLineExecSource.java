package com.dataknown.flume.source;
...
/**
 * Created by ureyqiao on 2016/3/21. contact me: qiaowei@pku.edu.cn
 */
public class MultiLineExecSource extends AbstractSource implements
	...
	public void start() {
		...
		runner = new ExecRunnable(shell, command, getChannelProcessor(),
				...
				bufferCount, batchTimeout, charset, regex);
	...
	public void configure(Context context) {
		...
		restartThrottle = context.getLong(
				ExecSourceConfigurationConstants.CONFIG_RESTART_THROTTLE,
				ExecSourceConfigurationConstants.DEFAULT_RESTART_THROTTLE);

		restart = context.getBoolean(
				ExecSourceConfigurationConstants.CONFIG_RESTART,
				ExecSourceConfigurationConstants.DEFAULT_RESTART);

		logStderr = context.getBoolean(
				ExecSourceConfigurationConstants.CONFIG_LOG_STDERR,
				ExecSourceConfigurationConstants.DEFAULT_LOG_STDERR);

		bufferCount = context.getInteger(
				ExecSourceConfigurationConstants.CONFIG_BATCH_SIZE,
				ExecSourceConfigurationConstants.DEFAULT_BATCH_SIZE);

		batchTimeout = context.getLong(
				ExecSourceConfigurationConstants.CONFIG_BATCH_TIME_OUT,
				ExecSourceConfigurationConstants.DEFAULT_BATCH_TIME_OUT);

		charset = Charset.forName(context.getString(
				ExecSourceConfigurationConstants.CHARSET,
				ExecSourceConfigurationConstants.DEFAULT_CHARSET));

		shell = context.getString(
				ExecSourceConfigurationConstants.CONFIG_SHELL, null);

		regex = context.getString(
				MultiLineExecSourceConfigurationConstants.REGEX,
				MultiLineExecSourceConfigurationConstants.DEFAULT_REGEX);
	...
	private static class ExecRunnable implements Runnable {
		...
		public ExecRunnable(String shell, String command,
				...
				int bufferCount, long batchTimeout, Charset charset,
				String regex) {
		...
		public void run() {
			do {
				...
				try {
					if (shell != null) {
						String[] commandArgs = formulateShellCommand(shell,
								command);
						...
						synchronized (eventList) {
							...
							if (m.find()) {
								...
								if (buffer.size() != 0) {
									...
									eventList.add(EventBuilder.withBody(total
											.getBytes(charset)));
		...
		private static String[] formulateShellCommand(String shell,
				String command) {
